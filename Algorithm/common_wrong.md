...
| 2️⃣ | `double`                                   | `1.0`         |
| 3️⃣ | `float`                                    | `1.0f`        |
| 4️⃣ | 整数提升（int / long long / unsigned long long） | `1LL` |
...
5.递归的设计：注意对本对象的操作是放在上一次递归还是本次递归，如果放在上一次递归则要注意头尾元素的特殊处理
...
用指针构造树时，想要构建指针之间的关系必须通过父节点指向子节点（或者反过来）慎用指针赋值！小心空指针传递！

指针本质上还是一个变量，其变量值是地址，操作指针本身的值（指针赋值）就是改变其指向的地址值，但没有改变地址所在变量，也不会连锁改变指向同一地址的其他指针

注意函数传递指针和传递指针引用的区别：传递指针仅仅是复制了此地址，可能改变地址处变量，但不会改变指针变量本身；传递指针引用时才会改变指针变量
```cpp
prev -> left = ptr1;
//这个语句的含义是：变量 prev -> left 的值赋值为 ptr1 当前变量值
//并不意味着 prev -> left 的值此后都需要查找 ptr1
ptr1 = ptr2;
//此时 ptr1 和 ptr2 指向同一节点， prev -> left 仍然是 ptr1 之前的值
```
## ListNode
1.头尾节点特殊处理,最好自定义双向链表使用safe节点保护

2.空指针不能解引用，注意遍历节点时while条件要严谨，保证body中被解引用的节点非空

3.注意自环以及头尾节点单独成环

4.注意删除、插入、移动节点时指针的修改顺序，拓扑序严谨
...
20.注意循环越界以及判断条件的先后顺序，保证安全的条件要写在前面，例如数组下标

21.变量的分工要明确，最好不要通过一个变量的值判断状态，例如通过买入金额是否为 0 来判断是否买入，实际上买入金额可以为 0

22.比较器参数规范写成 `常量 + 引用` ,对性能优化大有裨益

23.图相关问题注意：自环，重边，孤立点 + 自环
