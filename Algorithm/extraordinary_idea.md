# 精彩的处理
算法设计中总有一些惊为天人的设计思路，避免复杂的讨论和边界，等价转换问题

### 延迟初始化
对所给数据进行处理时，不会一次性把数据全放入容器

在相关条件和输入顺序有关时，边输入边判断（注意不要忘了后续无关输入的读入）

在需要避免 `元素自身性检查`的题目中，`处理后再插入元素` 即可避免其与本身操作

典例：LeetCode_1_两数之和


#### 两数之和模型
- 对于当前元素，寻找可以与之配对的元素个数
- 使用哈希表存储某特征对应的元素个数
- 防止重复配对：延迟初始化、回溯（递归结尾将哈希计数减1）
- 变式：和为目标的连续元素（数组、树，使用前缀和）


### 分组降维
对于n组数据，要从每个一维数据中选一个，朴素的做法是 `嵌套遍历` n组数据并检验,这必然造成复杂度指数级上升，如果各组数据之间的处理可以以某种方式 `独立` 进行，就能避免嵌套  

其中一种方式就是分组，例如对于四组数据，两两分组，前两组一个 `O(n^2)`,并把结果捆绑在 `unorder_map` 中，后两组遍历并查询 `unorder_map`，即可完成，这样整体复杂度就降为了 `O(n^2)`
### 如何构造不重复答案
如果答案要求返回若干元组并且不能重复，采用多指针法，从排序数组头、尾分别出发， **保证 `a <= b <= c`, 并且每次与上一次不一样**，就可以保证不出现重复答案，并且使用了分组降维
```cpp
//四数之和
vector<vector<int>> fourSum(vector<int>& nums, int target) {
    sort(nums.begin(),nums.end());
    int n=nums.size();
    vector<vector<int>> ans;
    for(int a=0;a<n-3;a++) {
        if(a>0&&nums[a]==nums[a-1])continue;
        for(int d=n-1;d-a>=3;d--) {
            if(d<n-1&&nums[d]==nums[d+1])continue;
            for(int b=a+1;b<d-1;b++) {
                if(b>a+1&&nums[b]==nums[b-1])continue;
                //c无需遍历，和b并列操作即可
                int c=d-1;
                //abd确定时，不需要考虑c是否重复因为只有一个答案
                while(b<c&&(long long)nums[a]+nums[b]+nums[c]+nums[d]>target)c--;
                if(b==c)break;
                if((long long)nums[a]+nums[b]+nums[c]+nums[d]==target)ans.push_back({nums[a],nums[b],nums[c],nums[d]});
            }
        }
    }
    return ans;
}
```
### 返回值选择
当函数返回指针时，可以利用返回的是否为空指针代替布尔值返回，方便检查是否查找到指针

函数返回值可以作为副作用传递状态，特别是树的后续遍历返回值，可以传递子树的状态给父节点(此时注意特判根节点返回值，因为它不向上传递状态)


### 逆向思维
在正向寻找可能导致多次重复时，不妨考虑逆向寻找

例如，寻找能到达边界的点，如果依次判断所有点是否符合条件，那么路径上的点就会重复，此时逆向思考，寻找从边界出发能到达的点，简化了思路

### 符合条件的元素
对于完成一个目标，可能有种等价的判定方式，注意实现效率

例如，目标是在元素集合中构建图的边，判定条件为 p，一种方式是遍历每一对元素，检验 p 是否成立；另一种方式是遍历每一个元素，应用条件 p ,构造对应元素，在哈希结构中查找是否存在

### 构建环形
在数组需要环形遍历时，如果最多一圈，可以考虑复制一份到原数组末尾构造环形

### 双向遍历
有关比较左右两边大小的问题，一般思路有：

1.单调栈找某一边第一个比较的数
 
2.遍历一次得到趋势数组 （升降趋势以及连续个数）

3.双向遍历，每次处理一个方向

### 分类讨论
对于某些难以处理限制条件，有时可以拆分为几个简单情况，分别讨论其解

例如在环形数组选取不相邻元素的最大和，首尾部分难以处理，但是实际上就是两个情况，要么包含首，要么包含尾，因此拆解成两个线性dp即可(这是一类互斥条件，可以拆分处理)

### 空间优化
1.使用多指针记录临时变量，局部处理，避免存储所有元素

2.传入参数无保护需求时，利用传入参数的空间，原地处理


### 增加成员
当类、结构体已经被定义并且不方便修改时，可以考虑使用哈希表为其增加新成员，只需维护一个新的哈希表

例如保存父节点信息、链表前向节点信息
```cpp
unordered_map<Type*,MemberType> ump;

```